    ...
    var ckbox2 = $('#checkAllFooter');
   	ckbox.change(function () {
    	...
    	if (ckbox.is(':checked') || ckbox2.is(':checked')){
    		$('#btnExport').attr('class','btn');
 			$('#btnDelete').attr('class','btn');
 			$('#btnSetInterview').attr('class','btn');
 			$('td').attr("style", "background-color: #E8E8E8");
    	}
    	else{
    		$('#btnExport').attr('class','btn disabled');
 			$('#btnDelete').attr('class','btn disabled');
 			$('#btnSetInterview').attr('class','btn disabled');
 			$('td').removeAttr('style');
    	}
	});
